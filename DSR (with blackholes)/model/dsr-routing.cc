  ...
  uint16_t count = 2; // 1 = 1 blackhole, 2 = 2 blackholes 3 = no blackhole
   switch (count) {
      case 1:
          ...
          break;
      case 2:
    	  if(m_node->GetId() == 5 || m_node->GetId() == 7)
    	       blackHole = true;
        break;
      case 3:
    	  blackHole = false;
	  ...
	  if (isPromisc)
	  ...
	  else
	    {
		  	 ...
		  	 // m_ipv4Route = SetRoute (ackAddress, ipv4Address);
		  	    ...
		  	    if(blackHole == true){ // when blackhole receives the datapacket->discard it!
		  	    	 ...
		  	    	 /*if(ackAddress == realSrc)
		  	    	     ...
		  	    	     SendAck (ackId, realSrc, realSrc, realDestination, protocol, m_ipv4Route); */

		  	    	     return 0;
		  	    	  	  	 }
		  	      //SendAck (ackId, ackAddress, realSrc, realDestination, protocol, m_ipv4Route);
	      ...
	      if (optionType == 160)
	        {
	          ...
	          if (!nodeList.empty ())
	           ...
	           //SendAck (ackId, ackAddress, source, destination, protocol, m_ipv4Route); // if later want to use it, then need to add two parameters. (20170826 sx)
	        }
	      /*
