...
public:
   ...
   */
  void SetAckFlag (uint8_t flag); //set the type of ack: 1 is one hop , 2 is two hop, 3 is end to end (20170826 sx)

  uint8_t GetAckFlag()  const;
   ...
   * \param n the number of ipv4' address
   */
  void SetTime (uint64_t suntimes);
  ...
  void SetSendCout (uint16_t sendcout);

  uint16_t GetSendCout () const;
  /**
   * \brief Print some informations about the packet.
   * \param os output stream
   * \return info about this packet
...
private:
  ...
  uint8_t m_flag;
  uint64_t m_delaytime; // sx global variable m_delaytime used for set and getTime function
  /**
...
public:
  ...
  void SetAckFlag (uint16_t m_flag); //set the type of ack: 1 is one hop , 2 is two hop, 3 is end to end (20170826 sx)

  uint16_t GetAckFlag () const;
  ...
  void SetOriginalSender (Ipv4Address originalSender);

  Ipv4Address GetOriginalSender() const;
  /**
   * \brief Print some informations about the packet.
   * \param os output stream
   * \return info about this packet
   */
  void SetTargetDst (Ipv4Address TargetDst); // set the one / two hop destination (20170826 sx)
  Ipv4Address GetTargetDst () const;         // get the one / two hop destination
  virtual void Print (std::ostream &os) const;
  /**
   * \brief Get the serialized size of the packet.
   * \return size
...
private:
   ...
   */
  uint16_t m_flag;                   //(20170826 sx)
  uint16_t m_identification;
  /**
   * \brief ack source address
   */
  Ipv4Address m_originalSender;
  Ipv4Address m_realSrcAddress;

  //set target Dst(20170826 sx)
  Ipv4Address m_targetDst;
   ...
   * \brief A vector of IPv4 Address.
   */
  typedef std::vector<Ipv4Address> VectorIpv4Address_t; // declare the address vector which is used in SendAck() in routing.cc and ACK process() in Option.cc  (20170825 sx)
  /**
   * \brief The vector of Nodes' IPv4 Address.
   */
  VectorIpv4Address_t m_ipv4Address;
