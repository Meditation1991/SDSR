...
uint8_t DsrOptionRreq::Process (Ptr<Packet> packet, Ptr<Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source, Ipv4Header const& ipv4Header, uint8_t protocol, bool& isPromisc, Ipv4Address promiscSource)
{
  ...
  if (dupRequest)
  ...
  else
    {
      ...
      if(node->GetId() == 34 || node->GetId() == 25){  // blackhole test let 32 and 25 be blackhole attack
  ...
  else
    {
	              ...
	              Ipv4Address realSrc =  nodeList.front();
	  	    	  Ipv4Address realDestination =  nodeList.back();
	  	     	  ...
	  	     	  dsr->SendAck (ackId, ackAddress, realSrc, realDestination, protocol, m_ipv4Route, nodeList,1,ackAddress,realSender);


	       if (flag == 1){ //test the one hop
	    	  uint16_t ackId = 0;
	    	  Ipv4Address realSender = ipv4Address;
	    	  ...
	    	  Ipv4Address realDestination =  nodeList.back();
	     	  ...
	     	  Ptr<Packet> ackp = packet->Copy ();
	     	  m_ipv4Route = SetRoute (ackAddress, ipv4Address);
	     	  dsr->SendAck (ackId, ackAddress, realSource, realDestination, protocol, m_ipv4Route, nodeList,flag,ackAddress,realSender);
	       }
	       ...
	       if (flag == 2){  //test the two hops ack
              if(nodeList.size() == 2)
	     	  ...
	     	  if(ackTargetAddress != "0.0.0.0"){
	     		...
	     		Ptr<Packet> ackp = packet->Copy ();
	     		m_ipv4Route = SetRoute (ackAddress, ipv4Address);
	     		dsr->SendAck (ackId, ackAddress, realSource, realDestination, protocol, m_ipv4Route, nodeList,flag,ackTargetAddress,realSender);
...
uint8_t DsrOptionRerr::DoSendError (Ptr<Packet> p, DsrOptionRerrUnreachHeader &rerr, uint32_t rerrSize, Ipv4Address ipv4Address, uint8_t protocol)
{
 ...
 NS_ASSERT (p->GetSize () == 0);
  ...
  newSourceRoute.SetNodesAddress (nodeList);
  newSourceRoute.SetAckFlag(3);
  newSourceRoute.SetTime(Simulator::Now().GetMilliSeconds());
  newSourceRoute.SetSendCout(0);
  nextAddress = newSourceRoute.GetNodeAddress (nextAddressIndex);
  ...
  if(ackFlag == 2){
	  if(targetDst == ipv4Address){//(20170831 sx)
		  ...
		  if(nexthop == "0.0.0.0"){
			  ...
			  std::map<uint16_t, std::vector<Ipv4Address>>::iterator it = m_ackPair.find(ackId);
			  if (it == m_ackPair.end()) {  //(2017831 sx form the datastructure the first pa is ackid and ths sec is the list of original Sender)
				  std::vector<Ipv4Address> vec;
			     vec.push_back(originalSender);
			     m_ackPair.insert(make_pair(ackId, vec));
			  }else{
				  std::vector<Ipv4Address>::iterator iter = std::find(it->second.begin(),it->second.end(),originalSender);
				  if(iter == it->second.end())
					  it->second.push_back(originalSender);
			  }
			  ...
			  if(newTargetDst != "0.0.0.0"){
				  ...
				  dsr->SendAck (ackId, nexthop, realSrc, realDst, protocol, m_ipv4Route,nodeList, ackFlag,newTargetDst,originalSender);
			  }else{

				  m_ipv4Route = SetRoute (nexthop, ipv4Address);
				  dsr->SendAck (ackId, nexthop, realSrc, realDst, protocol, m_ipv4Route,nodeList, ackFlag,nexthop,originalSender);
			  }
		  }
  	  }
  	  else{
  		  ...
  		  dsr->SendAck (ackId, nexthop, realSrc, realDst, protocol, m_ipv4Route,nodeList, ackFlag,targetDst,originalSender);
     }
  }
  if(ackFlag == 3){
 ...
 * (20170826 sx)
if(ipv4Address == nodeList.front()){  // if the self address is the destination then show the message below
	std::cout<<"Now the ack is back to the source!\n\n";
dsr->UpdateRouteEntry (realDst);
dsr->CallCancelPacketTimer (ackId, ipv4Header, realSrc, realDst);
}else{  //go on to send the ack to the previous hop
	Ipv4Address nexthop = ReverseSearchNextHop(ipv4Address,nodeList);

	        std::cout<<"the current node: "<<ipv4Address<<", the previous node ip is "<<nexthop<<".\n";
	        Ptr<Packet> ackp = packet->Copy ();

	        std:: cout<<"the ack id is "<< ackId<<".\n";
	        m_ipv4Route = SetRoute (nexthop, ipv4Address);
	        std::cout<<"Send back ACK to the previous hop " << nexthop << " from the current node " << ipv4Address<<".\n\n";
	        dsr->SendAck (ackId, nexthop, nodeList.front(), realDst, protocol, m_ipv4Route, nodeList);
}*/
  return ack.GetSerializedSize ();
}
